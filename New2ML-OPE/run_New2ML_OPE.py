...
class runNew2MLOPE:
    ...
    def run(self):
        ...
        new2ml_ope = New2ML_OPE.New2MLOPE(self.settings['num_terms'], self.settings['num_topics'], self.settings['alpha'], 
        ...
        while i < self.settings['iter_train']:
            ...
            while True:
                ...
                (time_e, time_m, theta) = new2ml_ope.static_online(wordids, wordcts)
                # Compute sparsity
                sparsity = utilities.compute_sparsity(theta, theta.shape[0], theta.shape[1], 't')
                # Compute perplexities
                LD2 = utilities.compute_perplexities_vb(new2ml_ope.beta, self.settings['alpha'], self.settings['eta'], 
                                                        self.settings['iter_infer'], self.test_data)
                # Search top words of each topics
                list_tops = utilities.list_top(new2ml_ope.beta, self.tops)
                # Write files
                utilities.write_file(i, j, new2ml_ope.beta, time_e, time_m, theta, sparsity, LD2, list_tops, self.tops, 
                                     self.model_folder)
            datafp.close()
        # Write settings
        print'write setting ...'
        ...
        utilities.write_topics(new2ml_ope.beta, file_name)
        # Finish
        print'done!!!'
